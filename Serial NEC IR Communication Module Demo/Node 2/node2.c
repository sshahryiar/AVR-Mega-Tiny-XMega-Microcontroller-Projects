...
{
     unsigned char s = 0;
     unsigned char present_T = 0xFF;
     unsigned char previous_T = 0;
     unsigned char dht_T = 0;
     unsigned char dht_RH = 0;
     unsigned char present_button_state = 0xFF;
     unsigned char previous_button_state = 0xFF;
     
     setup();
     
     Lcd_Out(1, 3, "IR Comms. Node 2");
     Lcd_Out(2, 1, "DS18B20 T/ C:");
     Lcd_Out(3, 1, "DHT11 Tmp/ C:");
     Lcd_Out(4, 1, "DHT11 R.H/ %:");
     ...
     {
        present_T = get_temp();
        present_button_state = (PINB & 0x0F);
        
        if((present_T != previous_T) || (present_button_state != previous_button_state))
        {
            if((present_T <= 99) && (present_T >= 0))
            {
                 print_C(17, 2, present_T);
            }
            else
            {
                present_T = 0xFF;
            ...
            previous_button_state = present_button_state;
        }

        if(UART_RX_buffer[sync_byte] == 0x66)
        {
           dht_T = UART_RX_buffer[data_byte_1];
           dht_RH = UART_RX_buffer[data_byte_2];

           if((dht_T >= 0) && (dht_T <= 99) && (dht_RH >= 0) && (dht_RH <= 99))
           {
              print_C(17, 3, dht_T);
              print_C(17, 4, dht_RH);
           }
        ...
        UART_send_data(sync, present_button_state, present_T);
        delay_ms(1000);
     };
}
